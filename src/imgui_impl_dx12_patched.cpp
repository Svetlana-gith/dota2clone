...
struct ImGui_ImplDX12_ViewportData
{
    ...
    ImGui_ImplDX12_ViewportData(UINT num_frames_in_flight)
    {
        ...
        for (UINT i = 0; i < NumFramesInFlight; ++i)
        {
            ...
            // PATCH: Keep initial sizes at 0 so buffers are created on first use
            // Setting non-zero sizes without creating buffers causes issues
            FrameRenderBuffers[i].IndexBuffer = nullptr;
            FrameRenderBuffers[i].VertexBuffer = nullptr;
            FrameRenderBuffers[i].VertexBufferSize = 0;
            FrameRenderBuffers[i].IndexBufferSize = 0;
        }
    }
...
// PATCH: Deferred release queue for safe GPU resource deletion
// Resources are kept alive for numFramesInFlight frames before being released
struct ImGui_ImplDX12_DeferredRelease
{
    ID3D12Resource* Resource;
    UINT FrameCount;  // Number of frames to wait before releasing
};
static ImVector<ImGui_ImplDX12_DeferredRelease> g_DeferredReleases;

// PATCH: Add resource to deferred release queue instead of immediate release
static void DeferredRelease(ID3D12Resource* res)
{
    if (!res)
        return;
    ImGui_ImplDX12_Data* bd = ImGui_ImplDX12_GetBackendData();
    UINT framesToWait = bd ? bd->numFramesInFlight + 2 : 5;  // Wait extra frames for safety
    ImGui_ImplDX12_DeferredRelease dr;
    dr.Resource = res;
    dr.FrameCount = framesToWait;
    g_DeferredReleases.push_back(dr);
}

// PATCH: Process deferred releases - call once per frame
static void ProcessDeferredReleases()
{
    for (int i = g_DeferredReleases.Size - 1; i >= 0; i--)
    {
        if (g_DeferredReleases[i].FrameCount == 0)
        {
            if (g_DeferredReleases[i].Resource)
                g_DeferredReleases[i].Resource->Release();
            g_DeferredReleases.erase(&g_DeferredReleases[i]);
        }
        else
        {
            g_DeferredReleases[i].FrameCount--;
        }
    }
}

// PATCH: Clear all deferred releases (for shutdown)
static void ClearDeferredReleases()
{
    for (int i = 0; i < g_DeferredReleases.Size; i++)
    {
        if (g_DeferredReleases[i].Resource)
            g_DeferredReleases[i].Resource->Release();
    }
...
void ImGui_ImplDX12_RenderDrawData(ImDrawData* draw_data, ID3D12GraphicsCommandList* command_list)
{
    ...
    // PATCH: Process deferred releases at the start of each frame
    ProcessDeferredReleases();

    // Create and grow vertex/index buffers if needed
    if (fr->VertexBuffer == nullptr || fr->VertexBufferSize < draw_data->TotalVtxCount)
    {
        // PATCH: Use deferred release instead of immediate release to avoid GPU sync issues
        DeferredRelease(fr->VertexBuffer);
    ...
    {
        // PATCH: Use deferred release instead of immediate release to avoid GPU sync issues
        DeferredRelease(fr->IndexBuffer);
...
void ImGui_ImplDX12_Shutdown()
{
    ...
    // PATCH: Clear deferred releases before shutdown
